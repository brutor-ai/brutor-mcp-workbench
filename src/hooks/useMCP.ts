...
export const useMCP = () => {
  ...
  const connect = useCallback(async (
    ...
    if (!serverBaseUrl || !apiKey) {
      throw new Error('Please provide both MCP Server URL and OpenAI API Key');
    }
    ...
    try {
      ...
      // Create clients with correct parameters, including health check option
      const client = new MCPClient(actualServerBaseUrl, actualMcpEndpointPath, authToken, enableHealthCheck);
      const openai = new OpenAIClient(apiKey, model, logCallback);
